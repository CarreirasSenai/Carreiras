<template>
    <v-btn class="bg-deep-purple-accent-4" @click="validarEditarVaga">
        Editar Vaga
    </v-btn>
                    ...
                    <v-row dense>
                        <v-col cols="12" md="4">
                            <v-text-field v-model="form.cep" label="CEP" v-mask="'#####-###'"
                                prepend-inner-icon="mdi-map-marker" :rules="[rules.geral]" maxlength="9"></v-text-field>
                        </v-col>
                        <v-col cols="12" md="4">
...
export default {
    ...
    methods: {
        ...
        validarEditarVaga() {
            if (this.Vagas.raw.status === 1) {
                alert('A vaga já foi aprovada, não é possível editar!')
            } else {
                this.dialog = true;
            }
        }
